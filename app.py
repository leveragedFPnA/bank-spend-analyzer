...
def find_column(dataframe, words):
...
def standardize(dataframe, source_file):
    ...
    debit_column = find_column(data, ["debit", "withdrawal", "charge"])
    credit_column = find_column(data, ["credit", "deposit"])
    ...
    if description_column:
        ...
        result["description"] = data[text_columns].fillna("").astype(str).agg(
            " ".join,
            axis=1,
        )
    ...
    if debit_column or credit_column:
        ...
        result["amount"] = debit.abs().where(debit.abs() > 0, credit.abs())
...
def parse_file(uploaded_file):
    ...
    if filename.endswith((".csv", ".txt")):
        ...
        return standardize(raw_data, uploaded_file.name)
...
def save_category_changes(database, original, edited):
    ...
    for _, row in edited.iterrows():
        ...
        if new_category == str(original_categories.get(key)):
            continue

        database.table("transactions").update(
            {"category": new_category}
        ).eq("transaction_key", key).execute()
...
if st.button("Save category changes", type="primary"):
    if database is None:
        st.error("Supabase is required to save changes.")
    else:
        try:
            ...
            st.error(f"Categories could not be saved: {error}")
